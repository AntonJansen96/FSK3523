...
std::vector<Atom> genlattice(real spacing, std::vector<size_t> const &boxsize)
{
    size_t idx = 1;
    std::vector<Atom> Atoms;

    for (real x = 0.1; x < boxsize[0]; x += spacing)
    {
        for (real y = 0.1; y < boxsize[1]; y += spacing)
        {
            for (real z = 0.1; z < boxsize[2]; z += spacing)
            {
                if (idx % 2 == 0)
